...
class Trajectory:
    def __init__(self, x=None, NPW=None, wp=[], NW=0):
        assert type(wp) == ca.MX or type(wp) == ca.DM or type(
            wp) == np.ndarray or type(wp) == list
        if x is not None:
        ...
        if type(wp) == ca.MX or type(wp) == ca.DM:
            ...
            raise Exception(
                'need valid waypointlist [wp] or number of waypoints [NW]!')
    ...
    def parse(self):
        ...
        self.u = self.x[1:self.N * self.NU + 1]
        self.u = self.u.reshape(-1, self.NU).T
        self.state = self.x[self.N * self.NU + 1:self.N * self.NU + 1 +
                            (self.N + 1) * self.NX]
        ...
        self.mu = self.x[self.N * self.NU + 1 +
                         (self.N + 1) * self.NX:self.N * self.NU + 1 +
                         (self.N + 1) * self.NX + (self.N + 1) * self.NW]
        self.mu = self.mu.reshape(-1, self.NW)
        self.mu = self.mu.T
        self.nu = self.x[self.N * self.NU + 1 + (self.N + 1) * self.NX +
                         (self.N + 1) * self.NW:self.N * self.NU + 1 +
                         (self.N + 1) * self.NX + (self.N + 1) * self.NW +
                         self.NW * self.N]
        self.nu = self.nu.reshape(-1, self.NW)
        self.nu = self.nu.T
        self.tau = self.x[self.N * self.NU + 1 + (self.N + 1) * self.NX +
                          (self.N + 1) * self.NW +
                          self.NW * self.N:self.N * self.NU + 1 +
                          (self.N + 1) * self.NX + (self.N + 1) * self.NW +
                          self.NW * self.N + self.NW * self.N]
        self.tau = self.tau.reshape(-1, self.NW)
        self.tau = self.tau.T
        ...
        for i in range(self.N):
            self.thrust[:, i] = np.array(
                rotate_quat(ca.DM(self.q[:, i]),
                            ca.vertcat(0, 0, ca.cumsum(self.u[:, i]))))[:, 0]
            self.dir[:,
                     i] = self.thrust[:, i] / np.linalg.norm(self.thrust[:, i])
    ...
    def unparse(self):
        ...
        idx = np.array(
            [0, *list(range(n_start + self.NU - 1,
                            len(self.x) - 1, n_slice))])
    ...
    def plotWaypoints(self,
        ...
        if len(data) == 2:
            ...
            p = ax.plot(wp[data[0], :], wp[data[1], :], wp[data[2], :], style,
                        **kwargs)
            o = ax.plot(self.p[data[0], [0]], self.p[data[1], [0]],
                        self.p[data[2], [0]], '.k')
        else:
        ...
        if len(data) == 2:
            ...
            if plot_ori:
                ...
                for i in range(len(idx)):
                    pdir += [
                        ax.arrow(px[i], py[i], arrow_size * dirx[i],
                                 arrow_size * diry[i], **arrow_args)
                    ]
            ...
            p += ax.plot(self.p[data[0], :], self.p[data[1], :],
                         self.p[data[2], :], **kwargs)
            smax = np.max(self.p)
            smin = np.min(self.p)
            if plot_ori:
    ...
    def plotThrust(self,
        ...
        for i in range(min(len(data), 3)):
            p += ax.plot(self.t_u,
                         self.m * self.a[data[i]],
                         astyle,
                         label='$f_%c$' % plot_axis[i],
                         **kwargs)
    ...
    def save(self, filename, readable=False):
        ...
        with open(filename, 'w') as csvfile:
            traj_writer = csv.writer(csvfile,
                                     delimiter=',',
                                     quotechar='"',
                                     quoting=csv.QUOTE_MINIMAL)
            if not readable:
                traj_writer.writerow(
                    [self.NW, self.NPW, self.NX, self.NU, self.N])
            ...
            if readable:
                labels = [
                    ...
                    'v_x', 'v_y', 'v_z', 'w_x', 'w_y', 'w_z', 'a_lin_x',
                    'a_lin_y', 'a_lin_z', 'a_rot_x', 'a_rot_y', 'a_rot_z',
                    'u_1', 'u_2', 'u_3', 'u_4'
                ]
                for i in range(self.NW):
                    labels += ['mu_' + str(i)]
                ...
                for i in range(self.N + 1):
                    # States
                    row = [
                        self.t_x[i], self.p[0, i], self.p[1, i], self.p[2, i],
                        self.q[0, i], self.q[1, i], self.q[2, i], self.q[3, i],
                        self.v[0, i], self.v[1, i], self.v[2, i], self.w[0, i],
                        self.w[1, i], self.w[2, i], self.a_lin[0, i],
                        self.a_lin[1, i], self.a_lin[2, i], self.a_rot[0, i],
                        self.a_rot[1, i], self.a_rot[2, i]
                    ]
    ...
    def load(self, filename):
        ...
        with open(filename, 'r') as csvfile:
            traj_reader = csv.reader(csvfile,
                                     delimiter=',',
                                     quotechar='"',
